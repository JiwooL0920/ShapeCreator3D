module HouseCacheTest exposing (main)
...
type alias MeshList = 
    { treeMeshes : Array (Entity WorldCoordinates)
    , treeDefault : Entity WorldCoordinates
    }
...
-- Speed should be times per second
repeatDuration : Float -> Int -> Int -> Float -> Int
repeatDuration speed duration startPosition time =
  modBy duration (floor (speed * time)) + startPosition

-- The idea here is to get an integer value from repeatDuration for use in accessing Array elements
--repeatMapToInt speed duration time = floor (repeatDuration speed duration 0 time / speed)

myMeshes : MeshList
myMeshes =  { treeMeshes = Array.fromList (List.map
                ( \ n -> generateTree (3*sin (degrees n) + 20) )
                (List.map toFloat <| List.range 0 360))
            , treeDefault = generateTree 100
            }

generateTree y = Scene3d.group [  treeRoot
                              , treeLeaves y 15 40
                              ]

treeRoot =  (Scene3d.cylinder (Material.matte Color.brown) <|
...
view model =
    let
        ...
        -- CACHING
        tree i = Maybe.withDefault model.meshes.treeDefault (Array.get i model.meshes.treeMeshes)

        treeOld y = Scene3d.group [  root
                              , leaves y 15 40
                              ]
    ...
    Scene3d.custom
        ...
        , entities =  [   plane
                        ...
                        , house
                        , clouds
                          |> Entity.translateBy (Vector3d.withLength (Length.centimeters (20*sin (100* model.time))) Direction3d.y)
                        , fences
                        -- CACHING
                        , tree (repeatDuration 36000 360 0 model.time)
                        ...
                        , basketBallCourt
                        , ball 
                            |> Entity.translateBy (Vector3d.withLength (Length.centimeters (abs <| 30*sin (3000* model.time))) Direction3d.z)
