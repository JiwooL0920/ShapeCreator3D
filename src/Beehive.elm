{- Beehive for the Bee Game
 -}

module Beehive exposing (main)
...
myEntities model =  
    [ beehive model 
      |> rotate 0 0 (degrees (5 * sin model.time))
      |> move (0,0,40)
    ]

beehiveSegment size model = ellipsoid 10 10 5 model |> matte (Color.hsl 0.1 1 0.5) |> scale size

beehive model = Scene3d.group
    [ ring 15 5 |> matte (Color.hsl 0.1 1 0.5)
    , beehiveSegment 2.5 model |> move (0,0,10)
    , beehiveSegment 3 model |> move (0,0,20)
    , beehiveSegment 3.5 model |> move (0,0,30)
    , beehiveSegment 3.5 model |> move (0,0,40)
    , beehiveSegment 3 model |> move (0,0,50)
    , beehiveSegment 2.5 model |> move (0,0,60)
    ]
...
polygon : Int -> Float -> Material.Textured WorldCoordinates -> Entity WorldCoordinates
polygon sides size material = 
    let
...
view model =
    let
        ...
        sceneLighting =
            Light.directional (Light.castsShadows True)
                { direction = Direction3d.xyZ (Angle.degrees -90) (Angle.degrees -60)
                , chromaticity = Light.sunlight
        ...
        softLighting =
            Light.soft
                ...
                , intensityAbove = Illuminance.lux 150
        ...
        camera =
            Camera3d.perspective
                { viewpoint =
                    Viewpoint3d.orbitZ
                        { focalPoint = Point3d.centimeters 0 0 40
    ...
    in
    Scene3d.custom
        { lights = Scene3d.twoLights sceneLighting softLighting
        , camera = camera
...
polygonMesh : Int -> Mesh.Textured WorldCoordinates
polygonMesh sides = 
    let
        ...
        points = List.map
            ( \ n -> 
                { position = Point3d.translateBy (Vector3d.withLength (Length.centimeters 1) (Direction3d.xy (Angle.degrees (angleDegrees * n)))) Point3d.origin
                , uv = (0,0) -- Textures not properly supported for this mesh
                }
            )
        ...
        polyOrigin = 
            let 
                default = { position = Point3d.origin, uv = (0,0) }
            in
