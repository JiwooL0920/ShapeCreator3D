...
polyCylinder : List (Float,Float) -> Float -> Material.Uniform WorldCoordinates -> Entity WorldCoordinates
polyCylinder points height material =
    let
        bottom = List.map 
            ( \ (x,y) -> 
                Point3d.centimeters x y 0
        ...
        top = List.map 
            ( \ (x,y) -> 
                Point3d.centimeters x y height
            )
            points
    in
