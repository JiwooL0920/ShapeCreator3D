...
view model =
    let
        ...
        ( firstLight, firstLightBall ) =
            pointLight
                { position = Point3d.centimeters 0 0 0
        ...
        texturesList = case model.textures of
            ...
            Nothing ->
                List.repeat 6 (Material.constant 
                (Color.hsl 
                    (
                    if (((sin(degrees(5*model.time))) > (sqrt(3)/2)) && ((sin(degrees(5*model.time))) < (1)))
                    then
                    ((2*sqrt(3)/36) + (1/2))
                    else
                    (180/360) + ((40/360)*sin(5*degrees(model.time)))
                    ) 
                    0.6
                    (0.8*sin(degrees(5*model.time)))
                    ))  
        ...
        baseEntities =
            [ firstLightBall
            |> 
            if (sin(5*degrees(model.time))) > 0
            then
            Scene3d.translateBy (Vector3d.centimeters (60*cos(5*degrees(model.time))) 0 (60*sin(5*degrees(model.time))))
            else
            Scene3d.translateBy (Vector3d.centimeters -1000 -1000 -1000)
    ...
    Html.div []
        [
            Scene3d.custom
                    ...
                    , background = Scene3d.backgroundColor (Color.hsl (200/360) 1 0)
